...
public class FunGameHitBlockHeader extends FunGameView {
  ...
  @Override protected void initConcreteView() {
  ...
  @Override protected void drawGame(Canvas canvas, int width, int height) {
  ...
  @Override protected void resetConfigParams() {
  ...
  private void drawRacket(Canvas canvas) {
    ...
    canvas.drawRect(racketLeft, controllerPosition, racketLeft + blockWidth,
        controllerPosition + controllerSize, mPaint);
  }
  ...
  private void makeBallPath(Canvas canvas, int width) {
    ...
    if (cx
        <= blockLeft
        + blockHorizontalNum * blockWidth
        + (blockHorizontalNum - 1) * DIVIDING_LINE_SIZE
        + BALL_RADIUS) { // 小球进入到色块区域
    ...
    if (cx + BALL_RADIUS >= racketLeft
        && cx - BALL_RADIUS < racketLeft + blockWidth) { //小球当前坐标X值在挡板X值区域范围内
